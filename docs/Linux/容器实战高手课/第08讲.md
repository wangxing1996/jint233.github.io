...
结合前面说的两个条件，函数 oom\_badness() 里的最终计算方法是这样的：
...
杀掉“某个进程”的选择标准，涉及到内核函数 oom\_badness()。具体的计算方法是 ：系统总的可用页面数乘以进程的 OOM 校准值 oom\_score\_adj，再加上进程已经使用的物理页面数，计算出来的值越大，那么这个进程被 OOM Kill 的几率也就越大。
