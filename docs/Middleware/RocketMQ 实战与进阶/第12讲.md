...
现在订单系统会将消息发送到 ORDER\_TOPIC 中，大数据这边需要将订单数据导入自己的计算平台，对用户、商家的订单行为进行分析。

### PUSH 与 PULL 模式选型

大数据这边只需订阅 ORDER\_TOPIC 主题就可以完成数据的同步，那是采用 PUSH 模式还是 PULL 模式呢？
...
![1](assets/20200822122306949.png)
...
![2](assets/20200822122306955.png)
