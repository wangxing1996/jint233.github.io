...
初始化 seata-server 数据库，涉及三张表：branch\_table、global\_table 和 lock\_table，用于存储全局事务、分支事务及锁定表相关数据，脚本位于 conf 目录下 db\_store.sql 文件中。
...
每个独立的业务库，都需要 undo\_log 数据表的支持，以便发生异常时回滚。分别在会员库，财务库和消息库三个库中分别执行如下脚本，写入 un\_log 表。
