...
**String consumeTimestamp **指定从什么时间戳开始消费，其格式为 yyyyMMddHHmmss，默认值为 30 分钟之前，该参数只在 consumeFromWhere 为 CONSUME\_FROM\_TIMESTAMP 时生效。** AllocateMessageQueueStrategy allocateMessageQueueStrategy**
...
消息消费进度，首先使用 [\[email protected\]](/cdn-cgi/l/email-protection) 为键，其值是一个 Map，键为 Topic 的队列序列，值为当前的消息消费位点。
